...
\
    va_list ap;\
	va_start(ap, fmt);\
	int tempin = sizeof(buf) - iBufLen - 2;\
	int tempout = vsnprintf(buf + iBufLen, tempin, fmt, ap);\
	if (tempout > tempin)\
		tempout = tempin;\
	iBufLen += tempout;\
    va_end(ap);\
...
string CCftLogger::_file_name(int index)
{
    ...
    {
        sprintf(szSuffix, "%s%d.log", _suffix.c_str(), index);
    }

    if(_shift_mode == _DATE_MODE)
    {    
        sprintf(szFile, "%s_%04d%02d%02d%s", _path.c_str(), _tm_now.tm_year + 1900, _tm_now.tm_mon + 1,  _tm_now.tm_mday, szSuffix);
    }
    else
    {
        sprintf(szFile, "%s%s", _path.c_str(), szSuffix);
    }
